...
#include "../agdbtools/info_handle.h"

/* Tests the info_handle_initialize function
 * Returns 1 if successful or 0 if not
 */
int agdb_test_tools_info_handle_initialize(
     void )
{
 ...
 * Returns 1 if successful or 0 if not
 */
int agdb_test_tools_info_handle_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	AGDB_TEST_RUN(
	 ...
	 agdb_test_tools_info_handle_initialize );

	AGDB_TEST_RUN(
	 "info_handle_free",
	 agdb_test_tools_info_handle_free );
