...
Else
{
	...
	If (-Not (Test-Path "${VSSolutionPath}"))
	{
		...
		Invoke-Expression -Command "& '${Python}' ${MSVSCppConvert} --output-format ${VisualStudioVersion} ${VSToolsOptions} msvscpp\libagdb.sln 2>&1" | %{ "$_" }
	}
}
If (${VisualStudioVersion} -eq "2026")
{
	$VSSolutionFile = "${VSSolutionPath}\libagdb.slnx"
}
Else
{
	$VSSolutionFile = "${VSSolutionPath}\libagdb.sln"
}
