...
void (*agdbtools_signal_signal_handler)( agdbtools_signal_t ) = NULL;
...
{
	static char *function = "agdbtools_signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( agdbtools_signal_signal_handler != NULL )
			{
				agdbtools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "agdbtools_signal_attach";
	...
	agdbtools_signal_signal_handler = signal_handler;

	if( SetConsoleCtrlHandler(
	     (PHANDLER_ROUTINE) agdbtools_signal_handler,
	     TRUE ) == 0 )
	{
		libcerror_error_set(
		 error,
...
#if defined( _MSC_VER )
	agdbtools_signal_initialize_memory_debug();
#endif
...
int agdbtools_signal_detach(
     libcerror_error_t **error )
{
	static char *function = "agdbtools_signal_detach";
	...
	agdbtools_signal_signal_handler = NULL;
