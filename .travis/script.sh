...
then
	docker run -t -v "${PWD}:/libagdb" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libagdb && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libagdb;
